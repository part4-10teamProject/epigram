...
export const postJWTToken = async (authCode) => {
  ...
  const res = await fetch(
    host === 'codeit-epigram.netlify.app'
      ? `https://oauth2.googleapis.com/token?code=${authCode}&client_id=${process.env.NEXT_PUBLIC_NETLIFY_GOOGLE_URI}&client_secret=${process.env.NEXT_PUBLIC_GOOGLE_CLIENT_AUTH_KEY}&redirect_uri=${process.env.NEXT_PUBLIC_NETLIFY_GOOGLE_URI}&grant_type=authorization_code`
